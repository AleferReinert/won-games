...
export const SignUp: Story = {
  play: async ({ canvasElement, step }) => {
    ...
    const inputName = await waitFor(() => canvas.getByRole('textbox', { name: 'Full name' }))
    const inputEmail = await waitFor(() => canvas.getByRole('textbox', { name: 'E-mail' }))
    const inputPassword = await waitFor(() => canvas.getByLabelText('Password'))
    const inputConfirmPassword = await waitFor(() => canvas.getByPlaceholderText('Confirm password'))
    const buttonSignUp = await waitFor(() => canvas.getByRole('button', { name: 'Sign up' }))
